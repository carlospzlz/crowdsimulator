function horizontalLeader (agentID, position, strength, maxStrength, velocity, state, attributes, inbox, neighbours)
	
	--PARALLEL LEADER
	stateAction = {}

	--HORIZONTALFLOCKFORCE AUXILIAR FUNCTION
	function horizontalFlockForce(position,attributes,neighbours,wc,ws,wa)
	...
	function horizontalGoingRight(agentID, position, strength, maxStrength, velocity, state, attributes, inbox, neighbours)
		...
		flockForce = horizontalFlockForce(position,attributes,neighbours,0.1,0.1,0)
	...
	then
		...
		return stateAction.horizontalGoingRight(agentID,position,strength,maxStrength,velocity,state,attributes,inbox,neighbours)
	else
